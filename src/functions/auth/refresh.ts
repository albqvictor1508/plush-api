...
import { generateAccessToken, generateRefreshToken, hashRefreshToken } from "src/common/auth";
...
export const refresh = async (token: string | null) => {
  ...
  const [user] = (await db.select({ email: users.email, id: users.id }).from(users).where(eq(users.id, session.userId)));
